...
def main():
    ...
    clients = txt2dict(fname_cli,codes,dates,'%Y-%m-%d',decimals,bools,skip,'\'',';')
    for cli in clients:
    ...
    documents = txt2dict(fname_doc,codes,dates,'%Y-%m-%d',decimals,bools,skip,'"',',')
    ...
    dops = txt2dict(fname_dop,codes,dates,'%Y-%m-%d',decimals,bools,skip,'"',',')
    for dop in dops:
