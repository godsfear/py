...
def ost2graf(conn,number,role,commit):
    role2pay = {
        'PRNCPL': 'DEBT',
        'PRNCPL_OVRD': 'OVERDUE_DEBT',
        'INTRST': 'REWARD',
        'INTRST_OVRD': 'OVERDUE_REWARD',
    ...
    for r in tab:
        ...
        qry = "SELECT id FROM loans.loan_repayment_schedule_item WHERE loan_repayment_schedule_id = " + str(r[2]) + " AND is_migrated_balance_item = TRUE AND target_date = '" + r[3].strftime('%Y-%m-%d') + "'::DATE AND repayment_type_id = (SELECT id FROM loans.repayment_type WHERE code = '" + role2pay[role] + "') AND is_delayed = " + ('TRUE' if role in ['PRNCPL_OVRD','INTRST_OVRD','PNLT_PRNCPL_OVRD','PNLT_INTRST_OVRD'] else 'FALSE')
        cur = query(conn,qry)
        chk = cur.fetchall()
        if len(chk) == 0:
            qry = "INSERT INTO loans.loan_repayment_schedule_item (id,accrued_amount,amount,amount_principal_after,amount_principal_before,deferred_amount,is_delayed,is_repaid,target_date,loan_repayment_schedule_id,repayment_type_id,is_migrated,is_migrated_balance_item) SELECT NEXTVAL('loans.loan_repay_sched_item_seq_id') AS id," + (str(r[1]) if role in ['INTRST','INTRST_OVRD','PNLT_PRNCPL_OVRD','PNLT_INTRST_OVRD'] else 'NULL') + " AS accrued_amount," + str(r[1]) + " AS amount," + str(aft) + " AS amount_principal_after," + str(bfo) + " AS amount_principal_before," + (str(r[1]) if role in ['INTRST','INTRST_OVRD','PNLT_PRNCPL_OVRD','PNLT_INTRST_OVRD'] else 'NULL') + " AS deferred_amount," + ('TRUE' if role in ['PRNCPL_OVRD','INTRST_OVRD','PNLT_PRNCPL_OVRD','PNLT_INTRST_OVRD'] else 'FALSE') + " AS is_delayed,FALSE AS is_repaid,'" + add2date(r[3],days=1).strftime('%Y-%m-%d') + "'::DATE AS target_date," + str(r[2]) + " AS loan_repayment_schedule_id,(SELECT id FROM loans.repayment_type WHERE code = '" + role2pay[role] + "') AS repayment_type_id,FALSE AS is_migrated,TRUE AS is_migrated_balance_item RETURNING id"
            if not commit:
                print(qry)
            cur = query(conn,qry)
            tab = cur.fetchall()
