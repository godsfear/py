...
def lconv(dest,loan,rate,beg_date,commit):
    head = {'Content-type':'application/json','Accept':'text/plain'}
    cfg = config('migration.json')
    conn = connect(cfg[dest])

    pogash = ['2860']
    laccts = ['1411','1417','1424','1740','1741','1879','1860','111021','111022','127025','127029','201021','217025','217029']
    ...
    lsumm = 0
    for r in (tab):
        ost.append({'СЧЕТ':r[5],'НПС':r[0],'ТИП':[r[1],r[2]],'СУММА':r[3],'ВАЛЮТА':r[4]})
    ...
    for x in ost:
        ...
        x.update({'ЭКВИВАЛЕНТ':round(x['СУММА'] * rate,2)})
        print(label,'-->',x['ЭКВИВАЛЕНТ'],'KZT')
        plan.append({'amount':x['ЭКВИВАЛЕНТ'],'amount_principal_after':0,'amount_principal_before':lsumm,'is_delayed':(True if (x['НПС'] in _pod) or (x['НПС'] in _ppr) else False),'target_date':beg_date,'is_migrated':False,'is_migrated_balance_item':True,'repayment_type_id':cfg['ACCT_ROLE'][x['НПС']]['pay_type']})
    ...
    for a in ost.copy():
        ...
        if commit:
            req = requests.post(url,auth=(cfg['target'][who]['LOGIN'],cfg['target'][who]['PASSWORD']),headers=head,json=acc)
            print('Открытие счета ' + a['ТИП'][1],req.status_code,req.reason)
            if (req.status_code != requests.codes.ok):
                print(req.text)
                sys.exit(1)
            doc = etree.XML(req.text)
        ...
        ops = {'code':'IMPORT_ACCOUNT_BALANCES','eventParameters':{'AMOUNT':str(a['ЭКВИВАЛЕНТ']),'CURRENCY':'KZT','ACCOUNT_CR':zacct,'ACCOUNT_DB':a['ACCOUNT_DB'],'VALUEDATE':beg_date.strftime('%d.%m.%Y'),'PURPOSE':'Конвертация','AGREEMENT_NUMBER':(loan + '_KZT'),'SUBACCOUNT_TYPE':cfg['ACCT_ROLE'][a['НПС']]['role']},'DB_CR':'DEBIT'}
        sum += a['ЭКВИВАЛЕНТ']
        if commit:
            req = requests.post(url,auth=(cfg['target'][who]['LOGIN'],cfg['target'][who]['PASSWORD']),headers=head,json=ops)
            print('Операция формирования остатка ' + a['ТИП'][1],req.status_code,req.reason)
            if (req.status_code != requests.codes.ok):
                print(req.text)
                sys.exit(1)
        else:
...
def main():
    ...
    lconv(who,loan,rate,beg_date,commit)
